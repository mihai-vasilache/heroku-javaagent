...
public class Agent {
	...
	public static void premain(String agentArgs, Instrumentation instrumentation) {
		...
		for(String anArg : args) {
			if(anArg.startsWith("interval")) {
				String[] intervalKV = anArg.split("=");
				interval = 1000 * Integer.parseInt(intervalKV[1]);
			}
		}
		agent = new Agent(instrumentation, userlog, linuxMem, legacyFmt, interval);
	}

	public Agent(Instrumentation instrumentation, boolean userlog, boolean linuxMem, boolean legacyFmt, long interval) {
		this.instrumentation = instrumentation;
		System.out.println("\t\t ***************** Memory Agent Started at " + interval / 1000 + " sec. interval. ********************");
		timer.scheduleAtFixedRate(new Reporter(userlog, linuxMem, legacyFmt), 60000, interval);
	}
	...
	public static class Reporter extends TimerTask {
		...
		private void nativeMemReport() throws Exception {

			String pid = ManagementFactory.getRuntimeMXBean().getName().split("@")[0];
			String commandString = "jcmd " + pid + " VM.native_memory summary";
			String[] cmd = {
				"/bin/sh",
				"-c",
				commandString
